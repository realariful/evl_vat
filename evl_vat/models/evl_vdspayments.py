...
class EvlPayments(models.Model):
    _name = 'evl.vdspayments'
    _description = 'VDS Payments'
    _rec_name = 'name'
    ...
    purchase_order = fields.Many2one('purchase.order',required=True)  
    ...
    def create(self, vals):
        if vals.get('name', ('New')) == ('New'):
           vals['name'] = self.env['ir.sequence'].next_by_code('evl.vdspayments') or _('New')
